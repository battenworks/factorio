...
local function new(player, storage_player, entity, item_type, view_name, button_name)
	...
	selection_container.style = "bwbd_train_selection_container"
	...
	station_header.style = "bwbd_train_station_header"

	local station_label = station_header.add { type = "label" }
	station_label.caption = { "bwbd.stations-caption" }
	station_label.style = "bwbd_train_station_header_label"
...
train_view.toggle = function(player, entity, item_type, view_name, button_name)
	...
	local storage_view = storage_player.elements.train_view

	if storage_view == nil then
		new(player, storage_player, entity, item_type, view_name, button_name)
	else
...
train_view.clear = function(player)
	...
	if storage_player.elements.train_view ~= nil then
...
train_view.configure_train = function(player)
	local storage_player = common.get_storage_player(player)
	local storage_view = storage_player.elements.train_view
	local selected_item = storage_view.main_container.selection_container[train_view.selection_button_name].elem_value or "none"
	...
	train_behavior.render_station_list(storage_view.main_container.station_list_container, schedule)

	local train = storage_player.entities.train_entity
