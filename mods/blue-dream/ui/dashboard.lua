...
local view_name = "bwbd_dashboard"

dashboard = {
	name = view_name,
}

local function new(player, storage_player)
	...
	storage_player.elements.blue_dream_dashboard = main_window

	local title_bar_caption = { "bwbd.dashboard-caption" }
	...
	items_tab.caption = { "bwbd.items-caption" }

	local fluids_tab = tabbed_pane.add { type = "tab" }
	fluids_tab.caption = { "bwbd.fluids-caption" }

	local fuel_stations_tab = tabbed_pane.add { type = "tab" }
	fuel_stations_tab.caption = { "bwbd.fuel-caption" }

	local ammo_tab = tabbed_pane.add { type = "tab" }
	ammo_tab.caption = { "bwbd.ammo-caption" }

	local potions_tab = tabbed_pane.add { type = "tab" }
	potions_tab.caption = { "bwbd.potions-caption" }
...
dashboard.toggle = function(player)
	...
	local storage_dashboard = storage_player.elements.blue_dream_dashboard

	if storage_dashboard == nil then
		new(player, storage_player)
	else
		storage_dashboard.destroy()
...
dashboard.clear = function(player)
	...
	if storage_player.elements.blue_dream_dashboard ~= nil then
